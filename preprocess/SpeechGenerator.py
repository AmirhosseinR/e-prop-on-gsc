...
class SpeechGen(tensorflow.keras.utils.Sequence):
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        for i, ID in enumerate(list_IDs_temp):
            ...
            # curX = np.load(ID)
            curX = wav.read(ID)[1].astype(np.int16)#.astype(float)
    ...
    def get_next_batch(self):
        audios, classes, list_IDs_temp = self.__getitem__(self.index_current_minibatch)
        self.index_current_minibatch += 1
        if self.index_current_minibatch > self.__len__():
            self.index_current_minibatch = 0
    ...
    def get_next(self):
        ...
        return audios, classes, list_IDs_temp